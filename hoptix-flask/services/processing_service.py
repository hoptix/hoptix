...
class ProcessingService:
    """Media processing coordination service (video and audio)."""
    ...
    def process_audio_from_local_file(self, audio_row: Dict, local_audio_path: str):
        """Process an audio file from a local file path with enhanced logging"""
        import os
        from datetime import datetime
        
        audio_id = audio_row["id"]
        file_name = audio_row.get("meta", {}).get("gdrive_file_name", "Unknown")
        file_size = os.path.getsize(local_audio_path) if os.path.exists(local_audio_path) else 0
        
        logger.info(f"🎵 Starting audio processing pipeline")
        logger.info(f"   📁 File: {file_name}")
        logger.info(f"   🆔 Audio ID: {audio_id}")
        logger.info(f"   📏 Size: {file_size:,} bytes")
        logger.info(f"   📍 Local path: {local_audio_path}")
        ...
        try:
            ...
            # 2) Step‑1 split
            logger.info(f"✂️ [2/6] Starting transaction splitting...")
            txs = split_into_transactions(segments, audio_row["started_at"], audio_row.get("s3_key"))
            logger.info(f"✅ [2/6] Transaction splitting completed: {len(txs)} transactions identified")
            ...
            # 4) persist transactions
            logger.info(f"💾 [4/6] Inserting {len(txs)} transactions into database...")
            tx_ids = insert_transactions(self.db, audio_row, txs)
            logger.info(f"✅ [4/6] Transactions inserted with IDs: {len(tx_ids)} records")
