...
**Note**: If a customer has ordered an item that could be upsold (sandwhich, chili dog, etc), then only include the upsell chances for items not part of the initial order. For example, if a customer orders a chili dog and drink, then the chance for upsell per json should only be for the fries. This is a hard rule and must be followed.
