...
class Settings:
    ...
    AI_FEEDBACK_MODEL: str = os.getenv("AI_FEEDBACK_MODEL", "gpt-5-nano")
...
class Prompts:
    ...
    1. Identify recurring *issues* — patterns of mistakes, missed upsells, or process gaps.  
    Only include issues that appear in **at least two transactions**.
    2. Identify recurring *strengths* — behaviors or techniques the operator does well.  
    Again, include only those seen in **multiple transactions**.
    3. Avoid contradictions — if a behavior appears both positive and negative, clarify the nuance
    (e.g., "good upsells when prompted, but inconsistent across calls").
    4. Provide **transaction IDs** (not indices) as evidence for each issue or strength.
    5. Keep everything grounded in the text; do not invent new examples.
    ...
    Here is the list of feedback entries:
