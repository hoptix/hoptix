...
def initialize_voice_pipeline(location_id: str, date: str) -> str:
    ...
    try:
        ...
        # Insert run record (only using columns that exist in schema)
        run_data = {
            "org_id": org_id,
            ...
            # Note: pipeline_type, started_at, metadata don't exist in schema
        }
        ...
        if response.data and len(response.data) > 0:
            ...
            logger.info(f"Created run ID: {run_id} for voice diarization")
            return run_id
        else:
...
def complete_voice_pipeline(run_id: str, results: Dict[str, int]):
    ...
    Args:
        ...
        results: Processing results dictionary (logged but not stored in runs table)
    """
    # Only update status - completed_at and metadata columns don't exist in schema
    update_data = {
        "status": "complete"
    }
    ...
    if response.data:
        logger.info(f"Run {run_id} marked as complete. Results: {results}")
    else:
...
def fail_voice_pipeline(run_id: str, error_message: str):
    ...
    Args:
        run_id: Run ID to update
        error_message: Error description (logged but not stored in runs table)
    """
    # Only update status - completed_at and error columns don't exist in schema
    update_data = {
        "status": "failed"
    }
    ...
    try:
        response = db.client.table("runs").update(update_data).eq("id", run_id).execute()
        if response.data:
            logger.error(f"Run {run_id} marked as failed. Error: {error_message}")
    except Exception as e:
